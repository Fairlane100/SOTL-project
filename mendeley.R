...
groups <- GET('https://api.mendeley.com/groups', config(token = token), verbose())
...
doc_rsp <- GET(paste('https://api.mendeley.com/catalog?view=stats&doi=', 
                     curlEscape(doi), sep=''), config(token = token))
