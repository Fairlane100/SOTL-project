...
myapp <- oauth_app(appname = 'funWithSOTL', key = "2363", secret = "dmI2oHj00OpHcqLY")
token <- oauth2.0_token(mendeley, myapp, scope='all',cache=TRUE)

profile_rsp <- GET('https://api.mendeley.com/profiles/me', 
                   config(token = token),
                   c(use_proxy("proxy.useastgw.xerox.com", port = 8000), verbose()))
